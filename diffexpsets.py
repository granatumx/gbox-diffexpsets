...
def main():
    ...
    for coli in mean_df:
        for colj in mean_df:
            if coli != colj:
                zscore_dfs.append((math.sqrt(size_df[coli])*(mean_df[coli]-mean_df[colj])/(std_df[coli]+std_df[colj])).fillna(0).clip(-20.0, 20.0))
